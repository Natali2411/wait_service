...
#@pytest.mark.skip
class TestAdjustRequestEditApprove():
    """Test-case CRT-1363"""
    ...
    def test_add_time_record_request_on_edit(self, log_obj, adjust_obj):
        try:
            adjust_obj.get_time_adjust_info_on_time_sheet_page_ui(tm_type="ClockInOut")
            adjust_obj.open_time_record_request_win_and_check(tm_type="ClockInOut", action="Update")
            adjust_obj.fill_time_rec_request_win(action="Update", tm_type="ClockInOut", rec_request_day_diff=1, rec_request_min_diff=10)
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_adjustment_edit_clock_in_out_request_creation(self, log_obj, clock_obj):
        try:
    ...
    def test_add_lunch_request_on_edit(self, log_obj, adjust_obj):
        try:
            adjust_obj.get_time_adjust_info_on_time_sheet_page_ui(tm_type="Lunch")
            adjust_obj.open_time_record_request_win_and_check(tm_type="Lunch", action="Update")
            adjust_obj.fill_time_rec_request_win(action="Update", tm_type="Lunch", rec_request_day_diff=1, rec_request_min_diff=10)
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_adjustment_edit_lunch_request_creation(self, log_obj, clock_obj):
        try:
            clock_obj.check_and_close_popup_message()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_adjust_request_info(self, log_obj, adjust_obj):
        try:
            adjust_obj.check_time_adjust_request(action="Update", state="Pending", time_manag_type="ClockInOut")
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_adjust_lunch_request_info(self, log_obj, adjust_obj):
        try:
            adjust_obj.check_time_adjust_request(action="Update", state="Pending", time_manag_type="Lunch")
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_approve_adjust_request(self, log_obj, clock_obj, adjust_obj):
        try:
            adjust_obj.confirm_adjust_request(decision="Approve", time_manag_type="ClockInOut", action="Update")
            clock_obj.check_and_close_popup_message()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
    ...
    def test_adjust_request_info_after_approve(self, log_obj, clock_obj, adjust_obj):
        try:
            ...
            adjust_obj.check_time_adjust_request(action="Update", state="Approved", time_manag_type="ClockInOut")
    ...
    def test_approve_adjust_lunch_request(self, log_obj, clock_obj, adjust_obj):
        try:
            adjust_obj.confirm_adjust_request(decision="Approve", time_manag_type="Lunch", action="Update")
            clock_obj.check_and_close_popup_message()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
    ...
    def test_adjust_lunch_request_info_after_approve(self, log_obj, clock_obj, adjust_obj):
        try:
            clock_obj.clock_in_out_location, clock_obj.clock_in_out_user = adjust_obj.rec_request_location, adjust_obj.rec_request_user
            adjust_obj.check_time_adjust_request(action="Update", state="Approved", time_manag_type="Lunch")
