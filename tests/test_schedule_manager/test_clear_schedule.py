...
class TestClearSchedule():
    ...
    def test_create_shifts_api(self, log_obj, schedule_obj):
        try:
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
                         is_published=True, start_hours_add=-4.1, end_hours_add=-3.9, duration="00:10:00")
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
                         is_published=True, start_hours_add=-2.8, end_hours_add=-2.4, duration="00:10:00")
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
                         is_published=True, start_hours_add=-1.35, end_hours_add=-1.1, duration="00:10:00")
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_clear_published_shifts_on_week_view(self, log_obj, schedule_obj):
        try:
    ...
    def test_clear_draft_shifts_on_week_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.clear_schedule_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_draft_shifts_after_reload_on_week_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.reload_page()
            schedule_obj.choose_location_to_clock_in(location="[ALL Stores]")
            schedule_obj.choose_employee(employee=config.user_full_name2)
            schedule_obj.apply_schedule_filter(action="Apply")
            schedule_obj.check_no_draft_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
            class_name, test_name = current_test[1], current_test[-1].split(' ')[0]
            log_obj.log_and_save_info(test_name=test_name, class_name=class_name)
    ...
    def test_create_shifts_api2(self, log_obj, schedule_obj):
        try:
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
                         is_published=True, start_hours_add=-4.1, end_hours_add=-3.9, duration="00:10:00")
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
    ...
    def test_clear_published_shifts_on_month_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.clear_schedule_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_clear_draft_shifts_on_month_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.clear_schedule_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_draft_shifts_after_reload_on_month_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.reload_page()
            schedule_obj.choose_location_to_clock_in(location="[ALL Stores]")
            schedule_obj.choose_employee(employee=config.user_full_name2)
    ...
    def test_create_shifts_api3(self, log_obj, schedule_obj):
        try:
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
                         is_published=True, start_hours_add=-4.1, end_hours_add=-3.9, duration="00:10:00")
            schedule_obj.create_shift_rows_api(user_name=config.user2, action_name="Time Clock Manager", is_access=1,
    ...
    def test_clear_published_shifts_on_day_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.clear_schedule_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_clear_draft_shifts_on_day_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.clear_schedule_shifts()
        except:
            current_test = os.environ.get('PYTEST_CURRENT_TEST').split('::')
    ...
    def test_draft_shifts_after_reload_on_day_view(self, log_obj, schedule_obj):
        try:
            schedule_obj.reload_page()
            schedule_obj.choose_location_to_clock_in(location="[ALL Stores]")
            schedule_obj.choose_employee(employee=config.user_full_name2)
